...
import './js/jquery.js';
let transWorker = new Worker('src/utils/js/transcode.worker.js');
//APPID，APISecret，APIKey在控制台-我的应用-语音听写（流式版）页面获取
const APPID = '9bef68ff';
const API_SECRET = 'OGVkYTJmZDZkMDNiY2JmYjA1NGM0NTg3';
const API_KEY = 'a865e13fbc0a39f82921e443fa42154e';
...
iatRecorder.onTextChange = function (text) {
  console.log('他说 --》', text); // sys-log
  // $('#result_output')[0].value = text;
  cbk(text);
};
...
export default (callback) => {
  ...
  if (iatRecorder.status === 'ing') {
    console.log('stop --->'); // sys-log
    iatRecorder.stop();
  } else {
    console.log('start --->'); // sys-log
    iatRecorder.start();
  }
};
